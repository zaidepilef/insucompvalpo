<?php /*%%SmartyHeaderCode:209882393556844e0fe36210-24034087%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '34d8caa485184b734dc0f70a7c71514ea7455fec' => 
    array (
      0 => '/home/insucomp/public_html/prestashop/themes/insucomp_1/modules/blocktopmenu/blocktopmenu.tpl',
      1 => 1451281939,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '209882393556844e0fe36210-24034087',
  'version' => 'Smarty-3.1.19',
  'unifunc' => 'content_609c8145573985_45759140',
  'has_nocache_code' => false,
  'cache_lifetime' => 31536000,
),true); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_609c8145573985_45759140')) {function content_609c8145573985_45759140($_smarty_tpl) {?>	<!-- Menu -->
	<div id="block_top_menu" class="sf-contener clearfix col-lg-12">
		<div class="cat-title">Menú</div>
		<ul class="sf-menu clearfix menu-content">
			<li class="sfHoverForce"><a href="http://prestashop.insucompvalpo.cl/13-toners" title="Toners">Toners</a></li>
					</ul>
	</div>
	<!--/ Menu -->
<?php }} ?>
