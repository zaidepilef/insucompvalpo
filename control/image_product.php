<?php
	...
	if (!$fileTmpLoc) { // if file not chosen
		...
		header("Location:main.php?action=product_edit&product_id=$product_id&error=1");
		exit();

	} else if($fileSize > 15602880) { // if file size is larger than 5 Megabytes

		//echo "ERROR: Your file was larger than 5 Megabytes in size.";
		header("Location:main.php?action=product_edit&product_id=$product_id&error=2");
		...
		header("Location:main.php?action=product_edit&product_id=$product_id&error=3");
		exit();

	} else if ($fileErrorMsg == 1) { // if file upload error key is equal to 1

		//echo "ERROR: An error occured while processing the file. Try again.";
		header("Location:main.php?action=product_edit&product_id=$product_id&error=4");
		exit();
	...
	$moveResult = move_uploaded_file($fileTmpLoc, "../upload_images/productos/$fileName_product");
	// Check to make sure the move result is true before continuing
	if ($moveResult != true) {
		//echo "ERROR: File not uploaded. Try again.";
		unlink($fileTmpLoc); 
		header("Location:main.php?action=product_edit&product_id=$product_id&error=5");
		// Remove the uploaded file from the PHP temp folder
		exit();
	}
	...
	$target_file	= "../upload_images/productos/$fileName_product";
	$resized_file	= "../upload_images/productos/product_$fileName_product";
	$wmax = 400;
	$hmax = 400;
	ak_img_resize($target_file, $resized_file, $wmax, $hmax, $fileExt,"yes");
	

	$SQL_LOAD_IMAGES = "INSERT INTO control_images (producto_id, imagen_nombre, imagen_tipo)VALUES (".$product_id.", '".$fileName_product."', 'product')";
